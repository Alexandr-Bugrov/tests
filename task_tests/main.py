...
documents = test_documents.copy()
directories = test_directories.copy()
...
def get_doc_owner_name(documents):
...
def add_new_shelf(shelf_number, directories):
    if not shelf_number:
...
def append_doc_to_shelf(doc_number, shelf_number, directories):
    add_new_shelf(shelf_number, directories)
...
def get_doc_shelf(directories, documents):
    user_doc_number = input('Введите номер документа - ')
    doc_exist = check_document_existance(user_doc_number, documents)
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
...
def move_doc_to_shelf(directories):
    user_doc_number = input('Введите номер документа - ')
